...
> [!WARNING]  
> The .pfx file's import password must be set in the APIs containers environment variable `ROOT_CERT_PASSWORD` (see [Todo]()). The easiest way to achieve this is to overwrite the `ROOT_CERT_PASSWORD` key's value in `build/result/docker/.env`.
