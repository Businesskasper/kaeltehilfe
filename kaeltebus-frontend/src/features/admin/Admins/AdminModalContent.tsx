...
type AdminLoginForm = AdminLoginPost & {
  password: string;
  password_: string;
...
const defaultAdminForm: AdminLoginForm = {
  role: "ADMIN",
  username: "",
  email: "",
  firstname: "",
...
type AdminLoginModalContentProps = {
  existing?: AdminLogin;
};
export const AdminLoginModalContent = ({
  existing,
}: AdminLoginModalContentProps) => {
  const {
    objs: { data: logins },
    post: { mutate: post },
  ...
  const closeModal = () => {
    modals.close("AdminLoginModal");
  };

  const onSubmit = (formModel: AdminLoginForm) => {
    if (existing) {
